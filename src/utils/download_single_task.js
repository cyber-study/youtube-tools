...
// const proxy_config = [];
const proxy_config = ["--proxy", "socks5://127.0.0.1:1080"];
const other_config = ["--cookies", path.resolve(__dirname, "../configs/youtube.com_cookies.txt")];

export default async function download_single(videoId) {
  try {
    const video_source_url = `https://www.youtube.com/watch?v=${videoId}`;
    const stdout_title = spawn(youtube_dl, ["--get-title", video_source_url, ...proxy_config, ...other_config]);
    ...
    const download_mission = spawn(youtube_dl, ["--format", "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best", "-o", download_dir_path, video_source_url, ...proxy_config, ...other_config], { cwd: process.cwd() });
